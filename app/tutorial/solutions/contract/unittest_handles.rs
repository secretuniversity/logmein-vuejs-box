...
mod tests {
    ...
    fn test_transfer() {
        ...
        let mut priv1 = Some(Metadata {
            token_uri: None,
            extension: Some(Extension {
                name: Some("MyNFT".to_string()),
                description: Some("privmetadata".to_string()),
                image: Some("privuri".to_string()),
                auth_key: Some([129, 149, 107, 182, 98, 47, 76, 231, 196, 74, 225, 27, 30, 228, 96, 255, 59, 20, 119, 233, 168, 100, 137, 76, 145, 97, 178, 120, 223, 86, 254, 140]),
                ..Extension::default()
            }),
        });
        let mut pub1 = Some(Metadata {
            token_uri: None,
            extension: Some(Extension {
                name: Some("MyNFT".to_string()),
                description: Some("pubmetadata".to_string()),
                image: Some("puburi".to_string()),
                auth_key: Some([156, 108, 73, 225, 85, 242, 153, 205, 112, 2, 118, 133, 58, 177, 62, 155, 0, 119, 231, 14, 120, 170, 151, 45, 230, 212, 168, 168, 102, 213, 240, 131]),
                ..Extension::default()
            }),
        });
        let execute_msg = ExecuteMsg::MintNft {
        ...
        priv1.as_mut().map(|p| p.extension.as_mut().map(|mut pext| 
            pext.auth_key = Some([122, 63, 213, 64, 218, 16, 161, 153, 208, 49, 206, 160, 22, 237, 166, 20, 22, 81, 10, 155, 62, 217, 179, 179, 167, 93, 105, 48, 213, 239, 145, 141])
        ));
        pub1.as_mut().map(|p| p.extension.as_mut().map(|mut pext| 
            pext.auth_key = Some([73, 240, 99, 70, 56, 16, 15, 128, 10, 253, 73, 70, 89, 230, 248, 138, 254, 66, 11, 123, 240, 58, 186, 21, 161, 216, 90, 190, 21, 63, 134, 47])
        ));
    ...
    fn test_send() {
        ...
        priv1.as_mut().map(|p| p.extension.as_mut().map(|mut pext| 
            pext.auth_key = Some([129, 149, 107, 182, 98, 47, 76, 231, 196, 74, 225, 27, 30, 228, 96, 255, 59, 20, 119, 233, 168, 100, 137, 76, 145, 97, 178, 120, 223, 86, 254, 140])
        ));
        pub1.as_mut().map(|p| p.extension.as_mut().map(|mut pext| 
            pext.auth_key = Some([156, 108, 73, 225, 85, 242, 153, 205, 112, 2, 118, 133, 58, 177, 62, 155, 0, 119, 231, 14, 120, 170, 151, 45, 230, 212, 168, 168, 102, 213, 240, 131])
        ));
