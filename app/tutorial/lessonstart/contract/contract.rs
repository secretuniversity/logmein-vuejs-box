...
pub fn metadata_generate_keypair(
    mut deps: DepsMut,
    sender: &Addr,
    env: Env,
    config: &Config,
...
fn mint_list(
    ...
    // let mut prng_seed = load(deps.storage, PRNG_SEED_KEY).unwrap();
    ...
    for mint in mints.into_iter() {
        ...
        // let (pubkey, scrtkey, new_prng_seed) = generate_keypair(
        //     env, 
        //     &deps.api.addr_humanize(sender_raw)?, 
        //     prng_seed, 
        //     None, 
        // );
    ...
    // save(deps.storage, PRNG_SEED_KEY, &prng_seed).unwrap();
