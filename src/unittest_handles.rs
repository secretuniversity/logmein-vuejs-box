...
mod tests {
    ...
    fn test_transfer() {
        ...
        let mut priv1 = Some(Metadata {
            ...
            extension: Some(Extension {
                ...
                description: Some("privmetadata".to_string()),
                image: Some("privuri".to_string()),
                auth_key: Some([128, 149, 107, 182, 98, 47, 76, 231, 196, 74, 225, 27, 30, 228, 96, 255, 59, 20, 119, 233, 168, 100, 137, 76, 145, 97, 178, 120, 223, 86, 254, 76]),
                ..Extension::default()
            }),
        });
        let mut pub1 = Some(Metadata {
            ...
            extension: Some(Extension {
                ...
                description: Some("pubmetadata".to_string()),
                image: Some("puburi".to_string()),
                auth_key: Some([129, 195, 132, 20, 8, 82, 120, 216, 160, 221, 190, 120, 184, 135, 222, 241, 232, 225, 175, 53, 209, 22, 45, 159, 169, 57, 142, 50, 196, 75, 236, 117]),
                ..Extension::default()
            }),
        });
        let execute_msg = ExecuteMsg::MintNft {
        ...
        priv1.as_mut().map(|p| p.extension.as_mut().map(|mut pext| 
            pext.auth_key = Some([120, 63, 213, 64, 218, 16, 161, 153, 208, 49, 206, 160, 22, 237, 166, 20, 22, 81, 10, 155, 62, 217, 179, 179, 167, 93, 105, 48, 213, 239, 145, 77])
        ));
        pub1.as_mut().map(|p| p.extension.as_mut().map(|mut pext| 
            pext.auth_key = Some([178, 112, 17, 80, 134, 82, 126, 194, 122, 155, 2, 141, 170, 58, 98, 57, 240, 224, 156, 180, 48, 79, 48, 219, 183, 83, 44, 82, 132, 76, 135, 119])
        ));
    ...
    fn test_send() {
        ...
        priv1.as_mut().map(|p| p.extension.as_mut().map(|mut pext| 
            pext.auth_key = Some([128, 149, 107, 182, 98, 47, 76, 231, 196, 74, 225, 27, 30, 228, 96, 255, 59, 20, 119, 233, 168, 100, 137, 76, 145, 97, 178, 120, 223, 86, 254, 76])
        ));
        pub1.as_mut().map(|p| p.extension.as_mut().map(|mut pext| 
            pext.auth_key = Some([129, 195, 132, 20, 8, 82, 120, 216, 160, 221, 190, 120, 184, 135, 222, 241, 232, 225, 175, 53, 209, 22, 45, 159, 169, 57, 142, 50, 196, 75, 236, 117])
        ));
